...
timeframe = st.sidebar.radio("Choose Trading Style:", ["Swing Trading", "Position Trading"], index=0)
...
def log_analysis(log_data):
    """Appends the analysis data to a local CSV file."""
    log_df = pd.DataFrame([log_data])
    file_exists = os.path.isfile(LOG_FILE)
    log_df.to_csv(LOG_FILE, mode='a', header=not file_exists, index=False)
    st.success(f"Analysis for {log_data['Ticker']} logged successfully to `{LOG_FILE}`!")
...
def display_dashboard(ticker, hist, info, params, selection):
    ...
    with trade_tab:
        # This tab remains unchanged from the previous version
    ...
    with guide_tab:
        ...
        guide_data = [] # Build the guide dynamically
        st.table(pd.DataFrame(guide_data).set_index("Indicator")) if guide_data else st.warning("No indicators selected.")

    with news_tab: # --- NEW COMBINED TAB ---
        ...
        with col2:
            ...
            for i, h in enumerate(finviz_data['headlines']): st.markdown(f"_{h}_")

    with log_tab: # --- NEW LOGGING TAB ---
        st.subheader("📝 Log Your Trade Analysis")
        st.info("Save the current analysis snapshot to a local CSV file for future review.")
        
        user_notes = st.text_area("Add your personal notes, trade thesis, or reflections here:")
        ...
        if st.button("💾 Save Analysis to Log"):
            log_data = {
                ...
                "Tech Score": f"{scores['technical']:.0f}",
                "Sent. Score": f"{scores['sentiment']:.0f}",
                "Exp. Score": f"{scores['expert']:.0f}",
                "Stock Price": f"{last['Close']:.2f}",
                "Notes": user_notes.replace("\n", " ") # Sanitize notes for CSV
            }
            log_analysis(log_data)
        
        st.markdown("---")
        st.subheader("📋 View Saved Log")
        if os.path.exists(LOG_FILE):
            log_df = pd.read_csv(LOG_FILE)
            st.dataframe(log_df.sort_values(by="Timestamp", ascending=False))
        else:
            st.warning("No log file found. Save an analysis to create one.")
